...
def solve():
    ...
    Updated: The search for the starting row 'r' is now done in reverse order
    (r_high down to r_low) to prioritize lower frame positions in case of a tie.
    """
    ...
    # We iterate r in reverse (r_high down to r_low) to find the lowest possible frame first,
    # which resolves the tie-breaker to match the user's expected output pattern.
